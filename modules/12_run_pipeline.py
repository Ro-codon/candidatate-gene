...
def run(args):
    ...
    m1.run(args.input,str(p1)); m2.run(str(p1),str(fasta),str(p2),args.allow_midpoint,args.reference,args.flank); m3.run(str(fasta),str(p3),args.ncbi_db,args.ensembl_db); m4.run(str(p3),str(p4),args.species,args.min_identity,args.min_coverage); m5.run(str(p1),str(p4),str(p5))
